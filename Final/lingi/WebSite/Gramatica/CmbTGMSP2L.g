...
atribuicao 
	: ^('=' ID {regCode("Pusha \"" + $ID.text +"\"") ;} expr {regCode("Store"); regLine($ID.line);})
	;

write 	: ^(WRITE expr {/*regCode("IOut");*/regLine($WRITE.line);})
	;
	
read 	: ^(READ ID {regCode("Pusha \"" + $ID.text + "\",IIn,Store"); regLine($READ.line);})
	;
