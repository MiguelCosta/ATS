...
cabecalho
		:	tipo ID '(' a1=argumentos? ')'	-> {a1==null}? ^(Cabecalho1 tipo ID)
							-> ^(Cabecalho2 tipo ID argumentos?)
		;

argumentos
		:	declaracao (',' declaracao)*	-> ^(Argumentos declaracao+)
		;	
...
declaracaoExpr	:	declaracao ('=' expr)? ';'	-> {expr==null}? ^(DeclaracaoExpr1 declaracao)
...
ifs		:	IF '(' expr ')' bloco ifsElse?	-> {ifsElse==null}? ^(Ifs1 expr bloco)
